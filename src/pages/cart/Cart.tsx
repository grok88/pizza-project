...
import {CartInitialStateType, clearCart, minusCartItem, plusCartItem, removeCartItem} from '../../redux/cart-reducer';
import emptyCartImage from '../../assets/img/empty-cart.png'
import {Link, NavLink} from 'react-router-dom';
import {Button} from '../../components';

const Cart: React.FC = () => {
    const {items, totalPrize, totalCount} = useSelector<AppRootStateType, CartInitialStateType>(state => state.cart);
    const dispatch = useDispatch();

    const addedPizzas = Object.keys(items).map(key => items[key].items[0]);

    const onClearCart = () => {
        if (window.confirm('Вы действительно жедвете удалить корзину?')) {
            dispatch(clearCart());
        }
    }
    ...
    const onOrderSend = () => {
        console.log('Ваш заказ оформлен', items)
    }
                ...
                ? <div className="cart">
                    ...
                    <div className="content__items">
                        {
                            addedPizzas.map(item => {
                                return < CartItem key={item.id}
                                                  ...
                                                  type={item.type}
                                                  size={item.size}
                                                  totalPrize={items[item.id].totalPrize}
                    ...
                    <div className="cart__bottom">
                        <div className="cart__bottom-details">
                            ...
                            <span> Сумма заказа: <b>{totalPrize} ₽</b> </span>
                        </div>
                        <div className="cart__bottom-buttons">
                            ...
                            <Button className=" pay-btn" onClickHandler={onOrderSend}>
